...
const ResultCard: React.FC<IResultCardProps> = ({link, image, title, domain})=>{
    ...
    <Card cursor="pointer" borderRadius="5px" width={!isMobile ? "237px" : "140px"}>
        <Image borderRadius="5px" width={!isMobile ? "237px" : "140px"} height={!isMobile ? "130px" : "95px"} src={image && image !== "" ? image : "/no_image_icon.png"} />
        <Flex position="relative" borderRadius="5px" bgColor="black" height={!isMobile ? "130px" : "95px"}>
            <Avatar bgColor="white" position="absolute" left="10px" top={isMobile ? "-12px" : "-16px"}
                    icon={getIcon()} size={isMobile ? "xs" : "sm"}/>

            <Center p="20px">
                <Text fontWeight={300} lineHeight={isMobile ? "14px" : "22px"}
